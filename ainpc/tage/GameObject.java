...
public class GameObject
{
	...
	*/
	...
	/** sets local pitch of object in degrees */

	public void setLocalPitch(float degrees) {
	    // Convert degrees to radians and create a rotation matrix around the local x-axis
	    localRotation.rotateLocalX((float) Math.toRadians(degrees));
	    update();
	}
	/** sets world yaw of object in degrees */

	public void setWorldYaw(float degrees) {
	    // Get the world up vector
    Vector3f worldUpVector = getWorldUpVector();
    // Create a quaternion representing the desired rotation around the world up vector
    Quaternionf rotationQuaternion = new Quaternionf().rotateAxis((float) Math.toRadians(degrees), worldUpVector.x(), worldUpVector.y(), worldUpVector.z());
    Matrix4f rotationMatrix = new Matrix4f().rotate(rotationQuaternion);
    localRotation.mul(rotationMatrix);
